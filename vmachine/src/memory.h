...
namespace mem {
    ...
    // using Ptr = uint32_t; // Есть класс вместо этого.

    // Сейчас страницы памяти растётут/удаляются как в стеке.
    // Мб сделаю динамическое выделение.
    class Memory {
    private:
        void* pages[256]; // TODO сделать динамическое выделение этого массива.
    ...
    public:
        ...
        int32_t* ToCPtr(const Ptr& ptr);
        int32_t& ToValue(const Ptr& ptr);
